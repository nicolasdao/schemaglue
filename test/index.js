...
describe('glue', () => {
	it('#01 BASIC - Should glue all schemas and resolvers resp. into a single schema and resolver object.', () => {
		const schema_321312 = `
		type UpdateMessage {
			message: String!
		}
	...
	it('#02 IGNORE FOLDER - Should ignore a folder while gluing.', () => {
	...
	it('#03 IGNORE ONE FILE - Should ignore a folder while gluing.', () => {
		const schema_cec82s = `
		type UpdateMessage {
			message: String!
		}
	...
	it('#04 IGNORE DIFFERENT FILES - Should ignore a folder while gluing.', () => {
	...
	it('#05 COMMENTS - Should be able to manage comments in the schema root.', () => {
		const schemaStr = `
		type Product {
			id: ID!
			name: String!
	...
	it('#06 TYPESCRIPT SUPPORT - Should support typescript resolvers.', () => {
		const { resolver } = glue('./test/graphql/mock_05', { mode: 'ts' })
		assert.isOk(resolver, 'resolver should exist.')
		assert.isOk(resolver.Query, 'resolver.Query should exist.')
		assert.isOk(resolver.Query.products, 'resolver.Query.products should exist.')
	})
	it('#07 CUSTOM RESOLVER GLOBBING - Should support custom file extensions for resolvers.', () => {
	...
	it('#08 SUPPORT .gql FILES - Should be glue .gql files.', () => {
		const schemaStr = `
		type Product {
			id: ID!
			name: String!
	...
	it('#09 SUPPORTS EMPTY TYPES - Should support empty types (ref: https://github.com/nicolasdao/schemaglue/issues/23).', () => {
		const schemaStr = `
		type Query {
			hello: String!
		}`
